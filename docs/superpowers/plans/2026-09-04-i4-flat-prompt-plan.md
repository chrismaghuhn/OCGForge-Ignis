...
**Architecture:** The contract is a clean-room semantic freeze over the pinned ocgcore prompt producers, EDOPro prompt readers, and EDOPro/core response path. A future I4 implementation will decode one complete prompt, resolve source references privately through the perspective mirror, copy published card locators and card codes only from a successful accepted I3D `PublicStateProjectionResultV1` (with `PublicSemanticLocatorV1` used only for syntax/correlation), retain a current-prompt-local private response binding, and emit exactly one original `CTOS_RESPONSE`; this slice creates no runtime seam.
...
- [x] **Step 2: Freeze public/private seams.** Define the exact `FlatPromptPublicContextV1` and `FlatPublicCandidateDescriptorV1` inclusion matrices, bind published card locators/card codes to the successful accepted I3D public-state result while retaining mirror-only private source resolution, define private current-prompt response binding, source ordinals, deterministic `i4_local_candidate_key` shape, the I6-owned OCGForge `public_action_key` boundary, no-auto-answer rule, and all forbidden metadata. A false POSITION card-code predicate is an absent conditional field, not a rejection of the validated mask domain.
...
- [x] **Step 4: Freeze future acceptance boundaries.** State that I4 is not implemented, I5/model input/I6 remain out of scope, no raw response or `ModernLocInfoV1` reaches public candidates, card-bearing references absent or ambiguous in the accepted public snapshot fail closed without a partial candidate list, and a valid multi-bit POSITION mask remains publishable when its unbound card code is absent.
...
- [x] **Step 3: Add binding vectors.** Cover stale same-looking keys from different prompt occurrences, current-prompt family mismatch, duplicate source entries remaining distinct, public/private storage separation without including private bindings in public candidate records, ambiguous/absent public-card correlation failing closed, paired internal-history equality, and a valid multi-bit POSITION mask whose unbound card code is absent without rejecting the domain.
...
- [x] **Step 4: Audit scope.** Run `git diff --check`, inspect `git diff --stat`, verify no `src/` or `tests/` file changed, search the documents for stale I4 implementation claims and mirror-owned public locators, then commit exactly the authorized files with the remediation-specific public-projection binding message.
