...
| EDOPro | 30935e847165a9ef0e547fb51a43f36168fab7c7 | `gframe/duelclient.cpp#DuelClient::ClientAnalyze` case `MSG_SELECT_CARD`; `gframe/client_field.cpp#ClientField::GetList`, `GetCard`; `gframe/client_card.cpp#ClientCard::SetCode` | The acting prompt reader consumes each SELECT_CARD source `u32` CardCode. A zero-location entry creates a temporary `limbo_temp` ClientCard and then applies a nonzero code; a location-bearing Deck entry resolves through the Deck list and receives the same code assignment. `SetCode` changes the client card code but does not itself establish public-state visibility. A zero wire code is not newly applied. | 2026-09-05 | PRIMARY prompt-local disclosure |
| ygopro-core | 46779fbe40e6a9bd8967f5dc6a03f4eaa6550d57 | `operations.cpp#field::select_tribute_cards`; `field.cpp#field::get_release_list`, `get_summon_release_list`; `playerop.cpp#field::process(Processors::SelectTributeP&)` | The operations entry forwards the generic min/max arguments, but the emitted SELECT_TRIBUTE semantics are weighted: `min` is the required accumulated release value and `max` is the maximum selected-card count. Eligible summon tributes receive release values 1, 2, or 3 from normal, double, or triple tribute effects; ordinary release-list entries receive 1. The prompt clamps max to five and to total release value, and validation checks selected count against max and weighted sum against min. | 2026-09-05 | PRIMARY weighted tribute semantics |
| EDOPro | 30935e847165a9ef0e547fb51a43f36168fab7c7 | `gframe/duelclient.cpp#DuelClient::ClientAnalyze` case `MSG_SELECT_TRIBUTE`; `gframe/event_handler.cpp#ClientField::OnEvent` card-selection branch; `gframe/event_handler.cpp#ClientField::SetResponseSelectedCards` | EDOPro stores the wire release value in `ClientCard::opParam`, compares selected-card count with `select_max`, compares the sum of `opParam` values with `select_min`, and sends the compact selected-index response. This independently confirms that SELECT_TRIBUTE `min` is not a minimum card count. | 2026-09-05 | PRIMARY client semantics |
| EDOPro | 30935e847165a9ef0e547fb51a43f36168fab7c7 | `gframe/duelclient.cpp#DuelClient::ClientAnalyze` case `MSG_SELECT_CARD`; `gframe/client_field.cpp#ClientField::ShowSelectCard` | The SELECT_CARD reader discards the selecting-player byte in this handler and uses the current client context to render the prompt. A nonzero prompt code is used for the card image even for a temporary zero-location card; perspective authorization is therefore a boundary the adapter must enforce, not something this reader proves for arbitrary observers. | 2026-09-05 | PRIMARY perspective-boundary fact |
| OCGForge | f13d19cab2b3677149f4caa25ef3c755623ded41 | `docs/contracts/decision-protocol-v1.md#SelectionContinuation` | The accepted OCGForge decision contract states: `Unordered set families use monotonic original candidate indices`, while ordered families retain remaining items and meaningful permutations. This is the relevant accepted contract phrase, but it belongs to OCGForge's own decision protocol and does not by itself resolve Ignis SELECT_SUM wire legality. | 2026-09-05 | PRIMARY accepted OCGForge contract |
| OCGForge | f13d19cab2b3677149f4caa25ef3c755623ded41 | `src/protocol/continuation.cpp#validate_items`, `add_monotonic_candidates`, `add_mask_candidates`, `add_candidates`, `apply_continuation_action`; `tests/protocol/continuation_oracle_test.cpp#test_unordered_and_zone_and_mask`, `test_tribute`, `test_sum` | OCGForge requires unique strictly increasing source indexes, rejects a pick at or below the last selected index for UnorderedSelection, Tribute, Sum, ZonePlacement, and AnnouncementMask, and deliberately exempts Ordering. The mask implementation uses the same monotonic rule for race and attribute message kinds, with message type selecting u64 versus u32 terminal encoding; the pinned exhaustive mask test directly instantiates race type 140, not a separate attribute type 141. Place monotonicity therefore depends on the caller's explicit place-order index vector. | 2026-09-05 | PRIMARY continuation implementation/evidence |
| ygopro-core | 46779fbe40e6a9bd8967f5dc6a03f4eaa6550d57 | `libgroup.cpp#LUA_FUNCTION(SelectWithSumEqual)`, `SelectWithSumGreater`; `processor_unit.h#SelectSum`; `playerop.cpp#select_sum_check1`, `field::process(Processors::SelectSum&)`; `field.cpp#field::check_with_sum_limit_m`, `check_with_sum_greater_limit_m` | The pinned core stores unrestricted Lua operation results in `uint32_t card::sum_param`, masks both 16-bit alternatives in feasibility helpers, serializes only `acc & 0xffff`, retains the original accumulator in an `int32_t` SelectSum processor, and final-validates through signed right-shift of the packed value without the feasibility mask. The prompt bytes therefore do not determine the unrestricted original accumulator and high alternatives consistently. SELECT_SUM exact legality remains unresolved. | 2026-09-05 | PRIMARY unresolved semantic blocker |
